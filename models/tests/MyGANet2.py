...
class Disp(nn.Module):
    ...
    def __init__(self, maxdisp=192):
        ...
        self.disparity = DisparityRegression(maxdisp=int(self.maxdisp/3))
...
class GANet(nn.Module):
    def __init__(self, maxdisp=192):
        ...
        self.maxdisp = maxdisp
        self.conv_start = nn.Sequential(BasicConv(3, 16, kernel_size=3, stride=3, padding=1),
                                        BasicConv(16, 32, kernel_size=3, padding=1),
                                        BasicConv(32, 32, kernel_size=3, padding=1))
        ...
        self.cost_agg = CostAggregation(int(self.maxdisp))
    ...
    def forward(self, x, y):
        ...
        if self.training:
            disp = list(disp)
            disp[0] = F.interpolate(torch.unsqueeze(disp[0],1), [x1.size()[2] * 3, x1.size()[3] * 3], mode='bilinear', align_corners=False)*3
            disp[0] = torch.squeeze(disp[0],1)
            disp[1] = F.interpolate(torch.unsqueeze(disp[1],1), [x1.size()[2] * 3, x1.size()[3] * 3], mode='bilinear', align_corners=False)*3
            disp.append(self.edge_refine(rem0, disp[1]))
            disp[1] = torch.squeeze(disp[1], 1)
        else:
            disp = F.interpolate(torch.unsqueeze(disp,1), [x1.size()[2] * 3, x1.size()[3] * 3], mode='bilinear',align_corners=False)*3
